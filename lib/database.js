...
Database.prototype.createTables = function () {
  this.db.serialize()
  this.db.run("CREATE TABLE IF NOT EXISTS users(uname TEXT, blacklisted TEXT, block TEXT, rank INTEGER, award INTEGER, primary key(uname))")
  ...
  this.db.run("CREATE TABLE IF NOT EXISTS userdata (username TEXT PRIMARY KEY  NOT NULL  UNIQUE , first INTEGER, last INTEGER, count INTEGER DEFAULT 0, sum INTEGER DEFAULT 0)")
  this.db.run("CREATE TABLE IF NOT EXISTS mcoins (username TEXT PRIMARY KEY NOT NULL UNIQUE, mcoins INTEGER, gold INTEGER, date TIMESTAMP, elsamagic INTEGER DEFAULT 0)")
  this.db.run("CREATE TABLE IF NOT EXISTS studios (uname TEXT PRIMARY KEY NOT NULL UNIQUE, limfamilyfarm INTEGER DEFAULT 0, chizu INTEGER DEFAULT 0, trashsum INTEGER DEFAULT 0, trigger INTEGER DEFAULT 0, papayafarms INTEGER DEFAULT 0, lastAwardDate TIMESTAMP)")
  this.updateTables()
};
  ...
  this.getVersion(function (version) {
    if (!version) {
      ...
      update.run(function () {
        ...
        self.db.run("ALTER TABLE users ADD award INTEGER")*/
        self.db.parallelize()
      })
    } else if (version === 1) {
      var update = self.db.prepare("UPDATE version SET 'value' = ? WHERE 'key' = ?", ['dbversion', '2'])
      update.run(function () {
        self.db.run("ALTER TABLE mcoins ADD elsamagic INTEGER DEFAULT 0")
        self.db.parallelize()
      })
    }
  })
...
Database.prototype.insertChat = function (msg, time, nick, room) {
  this.logger.syslog.log("Chat time: " + time);
...
// Inserts first-time user stats into the userdata table
// data - contains info such as user's name and time joined
Database.prototype.insertUserData = function(data) { 
  var stmt = this.db.prepare("INSERT OR IGNORE INTO userdata VALUES (?, ?, ?, 0, 0)", [data['name'], data['time'], data['time']]);
  stmt.run();
...
Database.prototype.getVideos = function (num, callback, query) {
  ...
  if (query)
    stmt = this.db.prepare("SELECT type, id, duration_ms, title FROM videos " +
      "WHERE flags = 0 AND (duration_ms BETWEEN 1 AND ?) AND (title LIKE ?) AND (type = 'gd' OR type = 'yt') " +
      "ORDER BY RANDOM() LIMIT ?", [this.maxVideoLength, query, num]);
  else
    stmt = this.db.prepare("SELECT type, id, duration_ms, title FROM videos " +
      "WHERE flags = 0 AND (duration_ms BETWEEN 1 AND ?) AND (type = 'gd' OR type = 'yt') " +
...
//Set ElsaMagic for a user
Database.prototype.setElsaMagic = function (username, value) {
  var normalize = function (s) {
    var n = Number(s);
    if (isNaN(n)) return 0;
    else return n;
  ...
  stmt.get(function (err, row) {
    var stmt;
    if (row)
      stmt = self.db.prepare("UPDATE mcoins SET elsamagic = ? WHERE username LIKE ?", [normalize(value), username])
    else
      stmt = self.db.prepare("INSERT INTO mcoins (username, mcoins, elsamagic) VALUES (?, ?, ?)", [username, 0, normalize(value)])
...
Database.prototype.getElsaMagic = function(username, callback) {
  var normalize = function (s) {
    var n = Number(s);
    if (isNaN(n)) return 0;
    else return n;
  ...
  stmt.get(function (err, row) {
    if (row) {
      var magic = {
        username: row.username,
        tier: normalize(row.elsamagic)
      };
      callback(magic);
    } else {
      callback({ username: username, tier: 0 });
    }
  })
}
...
Database.prototype.checkMCoins = function (username, callbackFunction) {
	...
	var stmt = this.db.prepare("SELECT * FROM mcoins JOIN studios ON username=uname WHERE username LIKE ?", [username]);
	var self = this;
	var date = new Date();
	...
	stmt.get(function (err, row) {
		if (row) {
			...
			var awardDate = new Date(row.lastAwardDate);
			var currDate = new Date().getTime();
			
			var ans = username + " has earned ";
			var coinAmt = false, goldAmt = false;

			if (start_of_day > ogDate.getTime()) {
				coinAmt = 10*(1+normalize(row.elsamagic));

				stmt = self.db.prepare("UPDATE mcoins SET mcoins = ?, date = ? WHERE username LIKE ?", [normalize(row.mcoins) + coinAmt, currDate, username]);
				stmt.run();
				
				ans += coinAmt + " coins";
			}

      let timeElapsed = currDate - awardDate.getTime();
      let lengthOfDay = 1000*60*60*24;

      // Over a day since last award
			if (timeElapsed >= lengthOfDay) {
        let days = Math.floor(timeElapsed/lengthOfDay);
				goldAmt = days*(0.3*normalize(row.limfamilyfarm)
          + 0.4*normalize(row.chizu)
          + 0.8*normalize(row.trashsum)
          + 1.2*normalize(row.trigger)
          + 2*normalize(row.papayafarms));
				
        // New date should be most recent date included in gold calc
				stmt = self.db.prepare("UPDATE mcoins SET gold = ?, date = ? WHERE username LIKE ?",[
          normalize(row.gold) + goldAmt,
          awardDate.getTime() + days*lengthOfDay,
          username
        ]);
				stmt.run();
				
				if (coinAmt != false) ans += " and ";
				ans += goldAmt + " gold";

				stmt = self.db.prepare("UPDATE studios SET lastAwardDate = ? WHERE uname LIKE ?", [currDate, username]);
				stmt.run();
			}

			coinAmt = normalize(row.mcoins) + coinAmt;
			goldAmt = normalize(row.gold) + goldAmt;

			if (ans != (username + " has earned ")) {
				ans += "! (Total: " + coinAmt + " coins, " + goldAmt + " gold)";
			...
			stmt = self.db.prepare("INSERT INTO mcoins (username, mcoins, date) VALUES (?,?,?)", [username, 10, currDate]);
			stmt.run();

			stmt = self.db.prepare("INSERT INTO studios (uname) VALUES (?)", [username]);
			stmt.run();
			
			callbackFunction("/judyhi " + username + "! You've earned 10 coins! (Total: 10 coins)");
			stmt.finalize();
			return;
		}
	});
...
Database.prototype.awardRaffle = function (amt, callback) {
  ...
  stmt.all(function (err, rows) {
    if (rows && rows.length) {
      var choosen = Math.round(Math.random() * (rows.length - 1));
      var winner = rows[choosen];
 
      callback({ username: winner.username, amount: prize })
      
      stmt = self.db.prepare("UPDATE mcoins SET mcoins=mcoins+? WHERE username=?", [prize, winner.username])
      stmt.run();
      stmt.finalize();
    } else {
...
Database.prototype.addgold = function (username, amt) {
  var normalize = function (s) {
    var n = Number(s);
    if (isNaN(n)) return 0;
    else return n;
  ...
  stmt.get(function (err, row) {
    var stmt;
    if (row)
      stmt = self.db.prepare("UPDATE mcoins SET gold = ? WHERE username LIKE ?", [normalize(row.gold) + normalize(amt), username])
    else
      stmt = self.db.prepare("INSERT INTO mcoins (username, gold) VALUES (?, ?)", [username, normalize(amt)])
...
Database.prototype.getgold = function(username, callback) {
  var normalize = function (s) {
    var n = Number(s);
    if (isNaN(n)) return 0;
    else return n;
...
Database.prototype.setStudio = function(username, studioname, tier, callback) {
	var studios = ["limfamilyfarm", "chizu", "trashsum", "trigger", "papayafarms"];
	if (studios.indexOf(studioname) == -1) callback(studioname + " doesn't exist -- you OK reatard?");
	...
	var stmt = this.db.prepare("SELECT * FROM studios WHERE uname = ?", [username]);
	var self = this;

	stmt.get(function (err, row) {
		var stmt;
		if (row)
			stmt = self.db.prepare("UPDATE studios SET " + studioname + " = ? WHERE uname LIKE ?", [normalize(tier), username]);
		else
			stmt = self.db.prepare("INSERT INTO studios (uname, " + studioname + ") VALUES (?, ?)", [username, normalize(tier)]);

		callback(username + " now has tier " + normalize(tier) + " of " + studioname);
		stmt.run();
		stmt.finalize();
		return;
	});
...
Database.prototype.getStudios = function(username, callback) {
	var normalize = function (s) {
		var n = Number(s);
		if (isNaN(n)) return 0;
		else return n;
	...
	var stmt = this.db.prepare("SELECT limfamilyfarm,chizu,trashsum,trigger,papayafarms FROM studios WHERE uname LIKE ?", [username]);
	...
	stmt.get(function (err, row) {
		if (row) {
			...
			callback({ username: username, studios: { "": "N/A" } });
		}
	})
}
