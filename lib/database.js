...
Database.prototype.createTables = function() {
    ...
    this.db.run("CREATE TABLE IF NOT EXISTS mcoins (username TEXT PRIMARY KEY  NOT NULL  UNIQUE , mcoins INTEGER)")
...
Database.prototype.addMCoins = function(username, amt) {
	var stmt = this.db.prepare("SELECT mcoins FROM mcoins WHERE username = ?", [username]);
	var newTotal;
	...
	stmt = this.db.prepare("UPDATE mcoins (mcoins) VALUES (?) WHERE username LIKE ?", [newTotal, username])
	stmt.run()
	stmt.finalize()
}
