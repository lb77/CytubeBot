...
Database.prototype.awardRaffle = function(amt, callback) {
  ...
  var date = new Date();
	var stmt = this.db.prepare("SELECT * FROM mcoins WHERE date >= ?", [date.getTime() - milliseconds_since_midnight(date)]);
	var self = this;
	
	stmt.get(function(err, row) {
		if(row) {
			var winner = row[Math.min(row.length -1, Math.floor(Math.random()*row.length))];
      console.log(winner);
