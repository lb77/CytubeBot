...
Database.prototype.giveCoins = function (amt, from, to, callback) {
    if (isNaN(Number(amt))) amt = 0;
    else amt = Number(amt);

    var stmt = this.db.prepare("SELECT * FROM mcoins WHERE name = ?", [from]);
    var self = this

    stmt.get(function (err, row) {
        if (row) {
            var stmt = self.db.prepare("SELECT * FROM mcoins WHERE name = ?", [to]);
            stmt.get(function (err, row) {
                if (!row) {
                    callback(to + "doesn't exist");
                    return;
                }
            });

            if (Number(row.mcoins) - amt < 0) callback(from+": You can't donate that many coins.");
            else {
                var transfer = .85 * amt;
                var tax = .15 * amt;
                stmt = self.db.prepare("UPDATE mcoins SET mcoins = mcoins - ? WHERE name = ?", [amt, from]);
                stmt.run();

                stmt = self.db.prepare("UPDATE mcoins SET mcoins = mcoins + ? WHERE name = ?", [transfer, to]);
                stmt.run();

                stmt = self.db.prepare("UPDATE mcoins SET mcoins = mcoins + ? WHERE name = 'CoinVaultofModship'", [tax])
                stmt.run();

                stmt.finalize();
                callback(from + " has donated " + amt + " coins to " + to);
            }
        }
        else {
            callback(from + " doesn't exist");
        }
    });
}
