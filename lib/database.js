...
Database.prototype.checkMCoins = function(username, callbackFunction) {
	...
	stmt.get(function(err, row) {
		if(row) {
			...
			if ((new Date().getTime() - ogDate.getTime()) >= ONE_DAY) {
				stmt = self.db.prepare("UPDATE mcoins SET mcoins = ? AND date = ? WHERE username LIKE ?", [normalize(row.mcoins)+10, new Date().getTime(), username])
...
Database.prototype.awardRaffle = function(amt, callback) {
	...
	var stmt = this.db.prepare("SELECT * FROM mcoins WHERE DAY(date) = ?", [new Date().getDay()])
	var self = this
