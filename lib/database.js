...
Database.prototype.checkMCoins = function (username, callbackFunction) {
	...
	stmt.get(function (err, row) {
		if (row) {
			...
			if ((date.getTime() - awardDate.getTime()) >= (1000*60*60*24)) {
				goldAmt = 0.3*normalize(row.limfamilyfarm) + 0.4*normalize(row.chizu) + 0.8*normalize(row.trashsum) + 1.2*normalize(row.trigger) + 2*normalize(row.papayafarms);
				
				stmt = self.db.prepare("UPDATE mcoins SET gold = ?, date = ? WHERE username LIKE ?", [normalize(row.gold) + goldAmt, currDate, username]);
				stmt.run();
