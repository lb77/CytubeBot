...
Database.prototype.checkMCoins = function(username) {
	var normalize = function (s) {
        var n = Number(s);
        if (isNaN(n)) return 0;
        else return n;
	...
	stmt.get(function(err, row) {
		if(row) {
			...
			if ((Date.now().getTime() - ogDate.getTime())) >= ONE_DAY {
				stmt = self.db.prepare("UPDATE mcoins SET mcoins = ? AND date = ? WHERE username LIKE ?", [normalize(row.mcoins)+10, Date.now(), username])
				ctx.answer(username + " has earned their 10 MCoins for the day! (Total: " + (normalize(row.mcoins)+10) + ")")
			}
		}
		else {
			stmt = self.db.prepare("INSERT INTO mcoins (username, mcoins, date) VALUES (?,?,?)", [username, 10, Date.now()])
			ctx.answer(username + " has earned their 10 MCoins for the day! (Total: " + (normalize(row.mcoins)+10) + ")")
		}
