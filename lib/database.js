  ...
  stmt.get(function (err, row) {
    if (row) {
      ...
      if (start_of_day > ogDate.getTime()) {
        stmt = self.db.prepare("UPDATE mcoins SET mcoins = ?, date = ? WHERE username LIKE ?", [normalize(row.mcoins) + 10 * (1 + normalize(row.elsamagic)), new Date().getTime(), username])
        callbackFunction(username + " has earned their 10 coins for the day! (Total: " + (normalize(row.mcoins) + 10 * (1 + normalize(row.elsamagic))) + ")")
      }
    } else {
...
Database.prototype.awardRaffle = function (amt, callback) {
  ...
  stmt.all(function (err, rows) {
    if (rows && rows.length) {
      ...
      stmt = self.db.prepare("UPDATE mcoins SET mcoins = mcoins + ? WHERE username LIKE ?", [prize, winner.username])
      // never do this
      /*
      stmt.run();
      stmt.finalize();
...
//O K, Liam-senpai wants gold now as an addition to coins gg
