...
function IRCClient(ircInfo, bot) {
	...
	this.client.addListener("action" + this.channel, function(from, to, text, message) {
		self.handleIRCAction(from, text)
	})
...
IRCClient.prototype.handleIRCMessage = function(from, message) {
	...
	this.bot.sendChatMsg((escape+"i"+escape) + from + ": " + message)
	if (message.indexOf("$") === 0 && from != this.nick && this.bot.loggedIn) {
		commands.handle(this.bot, "irc"+from, message, true)
	}
};

IRCClient.prototype.handleIRCAction = function(from, message) {
	var escape = "​";
	
	this.bot.logger.cytubelog.log("IRC Message: " + from + ": " + message)
	this.bot.sendChatMsg("/me " + (escape+"i"+escape) + from + ": " + message)
	if (message.indexOf("$") === 0 && from != this.nick && this.bot.loggedIn) {
		commands.handle(this.bot, "irc"+from, message, true)
	}
};
