...
var APIs = {
	...
	"socketlookup": function(serverData, apiKeys, callback) {
		var excellentServerRegex = /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\:?(\d*)?\/?$/
		var matches = serverData.server.match(excellentServerRegex)

		var options = {
			host: matches[2] + "." + matches[3],
			port: matches[5] !== undefined ? matches[5] : 80,
			path: "/socketconfig/" + serverData.room + ".json",
			timeout: 20
		}

                request({url: "http://"+options.host+options.path, json: true}, function (error, res, data) {
			// If we can't find the URL there's something wrong and
			// we should exit
			if (res.statusCode !== 200) {
				console.log("!~~~! Error looking up Cytube server info")
				process.exit(1)
			}
			console.log(data);
			...
			for (server in data.servers) {
				var server = data.servers[server]
				if (server.secure === false && server.ipv6 === undefined) {
					ipv4_nossl = server.url
				}
			}
