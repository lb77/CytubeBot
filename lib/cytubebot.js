...
CytubeBot.prototype.getUserData = function(nick) {
	var bot = this
	this.db.getUserData(nick, function(user) {
        var first;
        if (user.first === 0)
            first = "Veteran User";
        else
            first = nicetime(user.first);
        var last = nicetime(user.last);
                
        var msg = "*First Seen*: " + first + "; *Last Seen*: " + last + "; *Count*: " + user.count + "; *Average*: " + (user.sum / user.count).toFixed(2) + "; *Total*: " + numberWithCommas(user.sum);
		bot.sendChatMsg("[Whois: " + nick + "] " + msg)
	})
};
