...
var rustLines = require("./data/1v1rusts.json");
...
CytubeBot.prototype.addFromMediaLink = function(url) {
    var parsed = utils.handle(this, "parseMediaLink", url);
    console.log(parsed);
    this.addVideo(parsed.type, parsed.id, -1, true);
};

CytubeBot.prototype.handleSchedule = function () {
	console.log("Updating Schedule. AutoSchedule " + (this.stats.autoSchedule ? "enabled" : "disabled"));
	...
	if (this.schedule.length > 0) {
		...
		if (this.stats.autoSchedule) {
			this.scheduleHandler = setTimeout(function () {
				if (next.length === 4 && next[3] && new Date().getTime() >= next[1]) {
					console.log("Adding " + next[0]);
					bot.addFromMediaLink(next[3]);
					bot.scheduleJumpToID = next[3];
				}
				setTimeout(function () {
					bot.handleSchedule();
				}, 10000);
			}, Math.max(next[1] - new Date().getTime(), 10000));
		}
	}
}
...
CytubeBot.prototype.handleAddMedia = function(data) {
    ...
    var index = utils.handle(this, "findIndexOfVideoFromUID", data["item"]["uid"]);
    if (!this.playlist.slice(0, index).filter(function (elem) { return elem.temp; }).length) {
...
CytubeBot.prototype.sendAssignLeader = function(user) {
    ...
    //Glitch where bot sees rank as -1 regardless of being mod or not
    if (rank < 0) rank = 2;
...
CytubeBot.prototype.get1v1Rust = function() {
	return rustLines[Math.floor(Math.random() * rustLines.length)];
}