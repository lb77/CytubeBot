...
var perms = {
    ...
    "getRank": function(bot, data) {
        bot.db.getUserRank(data.name, data.callback)
    },
	...
	// rank - The rank the user should have
	// permission - The permission to look up 
	"checkAward": function(bot, data) {
		var username = data["username"]
		var award = data["award"]
        var rank = data["rank"]
		var callback = data["callback"]

		bot.db.getUserAward(data["username"], function(userAward) {        
			if (userAward >= award)
				callback(true, userAward)
			else
				bot.checkPermission(username, rank, null, function (val) { callback(val, userAward) })
		})
	},
    
    "setAward": function(bot, data) {
        var name = data["name"]
        var award = data["award"]
        
        bot.db.insertUserAward(name, award)
    },
    
    "getAward": function(bot, data) {
        bot.db.getUserAward(data.name, data.callback)
    },
