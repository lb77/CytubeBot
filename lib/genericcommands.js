...
var chatHandlers = {
    ...
    "translate": function (ctx, username, data) {
        if (data && (ctx.bot.mstranslateclient && ctx.bot.mstranslatesecret)) {
            if ((new Date().getTime() - ctx.bot.timeSinceLastTranslate) / 1000 < 5) {
                return ctx.bot.sendPM(username, "Translate cooldown");
            }
            ctx.bot.timeSinceLastTranslate = new Date().getTime();
            ...
            var query = {
                ...
                apikeys = {
                    clientid: ctx.bot.mstranslateclient,
                    secret: ctx.bot.mstranslatesecret
                };
            api.APICall(query, "translate", apikeys, function (data) {
                if (data.indexOf("must be a valid language") !== -1) {
                    return ctx.bot.sendPM(username, "Error: One or more invalid languages");
                }
                if (!from) {
                    return ctx.bot.sendPM(username, "[" + to + "] " + data);
                }
                ctx.answer("[" + from + "->" + to + "] " + data);
            });
        }
    },
    ...
    "wolfram": function (ctx, username, query) {
        ...
        if (ctx.bot.wolframLimiter.getTokensRemaining() < 1) {
            return ctx.bot.sendPM(username, "Wolfram allowance used up for the day");
        }
        api.APICall(query, "wolfram", ctx.bot.wolfram, function (result) {
            ctx.answer(result.replace(/\//gi, "/\ufeff").replace(/\^/gi, "^\ufeff").replace(/\>/gi, "\ufeff>"));
        });
    },
