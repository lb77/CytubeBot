...
var timegate = function (bot, username, award, rank, command, isPrivate) {
    ...
    if (isNumber(permLevel) && permLevel > -60 && isPrivate ||
        ...
        if (permLevel === -60)
            return [false, ["You used this command " + nicetime(then),"You can use this command again in " + nicetime(rankLevel + then)]];
        else if (rankLevel === -60)
            return [false, ["You used this command " + nicetime(then),"You can use this command again in " + nicetime(permLevel + then)]];
        else
            return [false, ["You used this command " + nicetime(then),"You can use this command again in " + nicetime(Math.min(permLevel, rankLevel) + then)]];
...
var chatHandlers = {
    ...
    "wolfram": function (ctx, username, query) {
        api.APICall(query, "wolfram", ctx.bot.wolfram, function (result) {
            ...
            ctx.answer("''t'" + utils.handle(this, "filterMsg", answer));
        });
    },
