...
var chatHandlers = {
    ...
    "autoschedule": function (ctx, username, data, fromIRC) {
        ...
        ctx.answer("AutoSchedule " + (ctx.bot.autoSchedule ? "enabled" : "disabled"));
    ...
    "addMCoins": function (ctx, username, data, fromIRC) {
        if (fromIRC) {
            return;
        }
        
        var name = data.trim().split(" ")[0];
        var amount = data.trim().split(" ")[0];
		
        ctx.bot.db.addMcoins(name, amount);
        ctx.answer(amount + " MCoins to " + name);
    },
    "getMCoins": function (ctx, username, name, fromIRC) {
            if (fromIRC) {
                    return;
            }
            ...
            var callback = function (row) {
                    ...
                    ctx.answer(nick + "has " + amt + " MCoins");
            }
            ctx.bot.db.getMCoins(name, callback)
    }
};
