...
var chatHandlers = {
	...
	"addMCoins": function (ctx, username, amount) {
		if (fromIRC) {
            return;
        }
		
		ctx.bot.db.addMcoins(username, amount);
		ctx.answer(amount + " MCoins to " + username);
	},
	"getMCoins": function (ctx, username) {
		if (fromIRC) {
			return;
		}
		...
		ctx.bot.db.getMCoins(username, callback)
	}
};
