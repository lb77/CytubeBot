...
var chatHandlers = {
	...
	"addrandom": function (ctx, username, data, fromIRC) {
		...
		if (num > 10 || num < 1) {
			num = Math.min(10, Math.max(num, 1));
		}
	...
	"choose": function(ctx, username, data) {
		...
		crypto.randomBytes(6, (err, buf) => {
			if (!err) {
				var choice = choices[buf.readUIntLE(0, 6) % choices.length];
	...
	"quotelist": function (ctx, username, nick) {
		ctx.bot.db.quotelist(nick.trim(), function (rows) {
		    if (rows.length === 0) {
		        // Empty PMs cause a kick
		        ctx.answer("No quotes found");
		    } else {
    			ctx.answer(rows.map(function (s) {
    				return "#" + s;
    			}).join(" "));
	...
	"autoschedule": function (ctx, username, data, fromIRC) {
		...
		ctx.bot.stats.autoSchedule = (data && data.trim().toLowerCase() === "true");
		ctx.bot.writePersistentSettings();
	...
	"setmagic": function (ctx, username, data, fromIRC) {
		if (fromIRC) return;
		...
		var name = data.trim().split(" ")[0];
		var value = data.trim().split(" ")[1];

		ctx.bot.db.setElsaMagic(name, normalize(value));
		ctx.answer("Elsa’s magic will now grant " + name + " an extra " + 10 * normalize(value) + " coins a day!");
	},
	"getmagic": function(ctx, username, name, fromIRC) {
		if (name == "") name = username.trim();
		else name = name.trim();
		...
		var callback = function (row) {
			...
			var nick = row.username;
			var tier = row.tier;
			ctx.answer(nick + " has tier " + tier + "  of Elsa's magic");
		}
		ctx.bot.db.getElsaMagic(utils.handle(ctx.bot, "filterMsg", name).replace("!", ""), callback);
	},
	"raffle": function (ctx, username, prize, fromIRC) {
		if (fromIRC) return;

		if (prize === undefined) 
			prize = Math.ceil(Math.random() * 41 + 9);
	...
	"tipgold": function (ctx, username, data, fromIRC) {
		if (fromIRC) return;
		...
		ctx.bot.db.addgold(name, normalize(amount));
		ctx.answer(amount + " gold pieces to " + name);
	},
	"getgold": function (ctx, username, name, fromIRC) {
		if (name == "") name = username.trim();
		else name = name.trim();
		...
		var callback = function (row) {
			...
			var nick = row.username;
			var amt = row.amount;
			ctx.answer(nick + " has " + amt + " gold");
		}
		ctx.bot.db.getgold(utils.handle(ctx.bot, "filterMsg", name).replace("!", ""), callback);
	},
	"setstudio": function (ctx, username, data, fromIRC) {
		if (fromIRC) return;
		...
		var uname = data.trim().split(" ")[0];
		var studioname = data.trim().split(" ")[1];
		var tier = data.trim().split(" ")[2];

		ctx.bot.db.setStudio(uname, studioname, normalize(tier), callback);
	},
	"getstudios": function (ctx, username, name, fromIRC) {
		if (name == "") name = username.trim();
		else name = name.trim();
		...
		var callback = function (row) {
			...
			var nick = row.username;
			var studios = row.studios;
			var str = "[" + nick + "'s Studios] ";

			for (var sname in studios) {
				str += sname + ": Tier " + studios[sname] + "; ";
			}
			ctx.answer(str);
		}
		ctx.bot.db.getStudios(utils.handle(ctx.bot, "filterMsg", name).replace("!", ""), callback);
	},
	"1v1rust": function (ctx, username, victimName, fromIRC) {
		if (victimName === "") return;

		// Using this to randomize name order
		let shuffle = function(arr) {
			let randIndex, currIndex = arr.length;
		...
		let line = ctx.bot.get1v1Rust();
		let namesArr = shuffle([username.trim(), victimName.trim()]);

		// Populate 1v1rust line ("? killed ? ...") with names
		line = line.replace(/\?/g, (_) => namesArr.pop());
		return ctx.answer(line);
	}
};
